...
out = tf.cond(tf.greater(x, y), lambda: 0.0, lambda: x + y)
print(sess.run(out))
...
out = tf.random_normal([10,10])
det = tf.matrix_determinant(out)
print(sess.run(out))
print(sess.run(det))
...
out = tf.cond(avg < 0, lambda: tf.reduce_mean(tf.squared_difference(x,y)), lambda: tf.reduce_sum(tf.abs(tf.subtract(x,y))))
print(sess.run(avg))
print(sess.run(out))
